  ...
  <div class="bg-grey vh-100">
    ...
    <div class="mt-12">
      ...
      <van-cell class="ptb-20 plr-20" :title="$t('me.userInfo.memberAccount')" value-class="light-grey" is-link title-class="black" :to="{ name: '' }">
        <template #default>
          <i class="iconfont icon-fuzhi fs-24 mr-10 vc"></i>{{ userInfo.memberAccount }}
        </template>
      </van-cell>
      <!-- 姓名 -->
      <van-cell class="ptb-20 plr-20" :title="$t('me.userInfo.name')" :value="userInfo.name" value-class="light-grey mr-10" is-link title-class="black" :to="{ name: 'me-setting-userinfoname', query: { name: userInfo.name, type: 'name' } }" />
      <!-- 昵称 -->
      <van-cell class="ptb-20 plr-20" :title="$t('me.userInfo.nickname')" :value="userInfo.nickname" value-class="light-grey mr-10" is-link title-class="black" :to="{ name: 'me-setting-userinfoname', query: { name: userInfo.nickname, type: 'nickname' } }" />
      <!-- 性别 -->
    ...
    <van-popup v-model="isPickerShow" round position="bottom">
      ...
      />
      <!-- 性别选择 -->
      <van-picker v-if="changeCurrentType === 'gender'" show-toolbar :columns="$t('common.gander')" @confirm="onConfirm($event, 'gender')" />
      <!-- 婚姻状态选择 -->
      <van-picker v-if="changeCurrentType === 'maritalStatus'" show-toolbar :columns="$t('common.maritalStatus')" @confirm="onConfirm($event, 'maritalStatus')" />
    </van-popup>
...
export default {
  ...
  methods: {
    ...
    onConfirm(val, key) { // 提交信息
      console.log(val, key)
      this.userInfo[key] = key === 'birth' ? this.$utils.formatStandardDate(val) : val;
