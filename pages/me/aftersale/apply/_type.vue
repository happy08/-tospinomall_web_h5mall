  ...
  <div class="bg-grey vh-100 pb-30">
    ...
    <div class="p-20 bg-white mt-12" v-if="$route.params.type == 2">
      ...
      <van-checkbox class="flex vcenter mt-20" @click="isGreenment = !isGreenment">
        <template #icon>
          <BmImage
            :url="isGreenment ? require('@/assets/images/icon/choose-icon.svg') : require('@/assets/images/icon/choose-default-icon.svg')"
            :width="'0.34rem'" 
            :height="'0.34rem'"
            :isLazy="false"
            :isShow="false"
    ...
    >
      ...
      <template #sku-stepper="props">
        <div class="tr plr-20 mt-24">
          ...
          <!-- <div class="mt-18" v-if="props.selectedSkuComb.stock_num">in stock: {{ props.selectedSkuComb.stock_num }}</div> -->
        </div>
      </template>
